  ...
  async getAgentStats() {
    try {
      ...
      const gameAgents = agents.filter(agent =>
        agent.type?.toLowerCase() === TYPE.GAME.toLowerCase()).length;

      const defiAgents = agents.filter(agent =>
        agent.type?.toLowerCase() === TYPE.DEFI.toLowerCase()).length;

      const socialAgents = agents.filter(agent =>
        agent.type?.toLowerCase() === TYPE.SOCIAL.toLowerCase()).length;
  ...
  async createRootstockAgent(createElizaAgentDto: CreateElizaAgentDto) {
    try {
      ...
      if (createElizaAgentDto.chain) {
        ...
        jsonObject.chain = 'opencampuscodex';
      }
