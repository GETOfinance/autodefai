...
const DefiAgentForm = () => {
  ...
  const [steps, setSteps] = useState<Step[]>([
    ...
    { text: "Deploying token to Open Campus Codex...", status: "pending" },
    { text: "Verifying contract on Open Campus Codex Explorer...", status: "pending" },
  ...
  async function onSubmit(values: z.infer<typeof AgentSchema>) {
    try {
      ...
      // Call the opencampuscodex endpoint instead of flow
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/create-agent/opencampuscodex`, {
        method: 'POST',
        headers: {
  ...
  const createTokenHandler = async () => {
    try {
      ...
      if (currentChainId !== 656476) {
        console.log('Not on Open Campus Codex, switching networks...');

        setSteps((prevSteps) => {
          const newSteps = [...prevSteps];
          newSteps[0] = {
            text: "Switching to Open Campus Codex...",
            status: "loading"
          };
          return newSteps;
        });
        ...
        try {
          ...
          if (switchError.code === 4902) {
            try {
              ...
              throw new Error('Could not add Open Campus Codex network to your wallet');
            }
          } else {
            throw new Error('Could not switch to Open Campus Codex network');
          }
        }
      }
      ...
      setSteps((prevSteps) => {
        ...
        newSteps[2] = { text: "Deploying token to Open Campus Codex...", status: "loading" };
        return newSteps;
      });
      ...
      setSteps((prevSteps) => {
        const newSteps = [...prevSteps];
        newSteps[2] = {
          text: `Token deployed to Open Campus Codex (${contract.target})`,
          status: "complete"
        };
        newSteps[3] = {
          text: `Verifying contract on Open Campus Codex Explorer...`,
          status: "loading"
        };
        return newSteps;
      });
      ...
      setTimeout(() => {
        setSteps((prevSteps) => {
          const newSteps = [...prevSteps];
          newSteps[3] = {
            text: `Contract verified on Open Campus Codex Explorer`,
            status: "complete"
          };
          return newSteps;
        });
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue="opencampuscodex">
                  <FormControl>
                    <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="opencampuscodex">Open Campus Codex</SelectItem>
                  </SelectContent>
                </Select>
                <FormDescription>Select the chain you want to deploy your agent on (Open Campus Codex)</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
